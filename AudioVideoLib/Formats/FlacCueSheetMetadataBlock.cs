...
public sealed class FlacCueSheetMetadataBlock : FlacMetadataBlock
{
    ...
    public override byte[] Data
    {
        ...
        protected set
        {
            ...
            for (var i = 0; (i < TrackCount) && (stream.Position < stream.Length); i++)
            {
                var trackOffset = stream.ReadBigEndianInt64();
                var trackNumber = stream.ReadByte();
                var trackIsrc = stream.ReadString(12);
                var flags = stream.ReadByte();

                // Per RFC 9639 §8.7: track-flags byte is bit-packed MSB-first as:
                ...
                List<FlacCueSheetTrackIndexPoint> indexPoints = [];
                for (var y = 0; (y < trackIndexCount) && (stream.Position < stream.Length); y++)
                {
                    var offset = stream.ReadBigEndianInt64();
                    var indexPointNumber = stream.ReadByte();
                    reserved = new byte[3];
