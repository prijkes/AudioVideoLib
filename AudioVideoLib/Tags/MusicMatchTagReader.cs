...
public sealed partial class MusicMatchTagReader : IAudioTagReader
{
    ...
    public IAudioTagOffset? ReadFromStream(Stream stream, TagOrigin tagOrigin)
    {
        ...
        else if (tagOrigin == TagOrigin.End)
        {
            ...
            var dataOffsets = ReadDataOffsets(sb);

            var startPosition = Math.Max(sb.Position + MusicMatchTag.FooterSize, 0);
            foreach (var audioMetaDataOffset in AudioMetaDataSizes.OrderBy(a => a))
            {
                // Seek to the start of the version information, based on the audioMetaDataOffset
                var startPositionHeader = Math.Max(startPosition - audioMetaDataOffset - MusicMatchTag.HeaderSize, 0);
                var endPositionHeader = startPositionHeader + HeaderIdentifierBytes.Length;
        ...
        // Unused, null padding
        var nullPadding = new byte[4];
        sb.Read(nullPadding, nullPadding.Length);

        // Read version info
        if (versionInformation == null)
        {
            versionInformation = ReadHeader(sb, TagOrigin.Start) ?? throw new InvalidDataException("Version information not found in the tag.");
        }
        else
        {
        ...
        if (tagOrigin == TagOrigin.Start)
        {
            ...
            foreach (var audioMetaDataOffset in AudioMetaDataSizes.OrderBy(a => a))
            {
                ...
                var startPositionFooter = Math.Max(startPosition + (audioMetaDataOffset - (sb.Position - startOffset) - (tag.UseHeader ? MusicMatchTag.HeaderSize : 0)), 0);
                var endPositionFooter = startPositionFooter + FooterIdentifierBytes.Length;
    ...
    private static MusicMatchHeader? ReadHeader(StreamBuffer stream, TagOrigin tagOrigin)
    {
        ...
        if (tagOrigin == TagOrigin.Start)
        {
            ...
            var endPositionHeader = Math.Min(startPositionHeader + HeaderIdentifierBytes.Length, streamLength);
            var header = ReadHeaderFooter(stream, startPositionHeader, endPositionHeader, HeaderIdentifierBytes, TagOrigin.Start);
            if (header != null)
            {
                return header;
            }
        ...
        else if (tagOrigin == TagOrigin.End)
        {
            ...
            var startPositionHeader = Math.Max(startPosition - HeaderIdentifierBytes.Length, 0);
            var endPositionHeader = Math.Min(startPositionHeader + HeaderIdentifierBytes.Length, streamLength);
            var footer = ReadHeaderFooter(stream, startPositionHeader, endPositionHeader, HeaderIdentifierBytes, TagOrigin.End);
            if (footer != null)
            {
                return footer;
            }
    ...
    private static MusicMatchHeader? ReadFooter(StreamBuffer stream, TagOrigin tagOrigin)
    {
        ...
        {
            // Look for a footer before the current position
            var startPositionFooter = Math.Max(startPosition, 0);
            var endPositionFooter = Math.Min(startPosition + FooterIdentifierBytes.Length, streamLength);
            var footer = ReadHeaderFooter(stream, startPositionFooter, endPositionFooter, FooterIdentifierBytes, TagOrigin.End);
            if (footer != null)
            {
                return footer;
            }
        ...
        {
            // Look for a footer before the current position
            var startPositionFooter = Math.Max(startPosition - MusicMatchTag.FooterSize, 0);
            var endPositionFooter = Math.Min(startPosition, streamLength);
            var footer = ReadHeaderFooter(stream, startPositionFooter, endPositionFooter, FooterIdentifierBytes, TagOrigin.End);
            if (footer != null)
            {
                return footer;
