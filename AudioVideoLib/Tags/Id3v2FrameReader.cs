...
public partial class Id3v2Frame
{
    ...
    public static Id3v2Frame? ReadFromStream(Id3v2Version version, Stream stream, long maximumFrameSize)
    {
        return stream == null
            ...
            : ReadFrame(version, stream as StreamBuffer ?? new StreamBuffer(stream), maximumFrameSize);
    }
    ...
    public static T? ReadFromStream<T>(Id3v2Version version, Stream stream, long maximumFrameSize) where T : Id3v2Frame, new()
    {
        return stream == null
            ? throw new ArgumentNullException("stream")
            : ReadFrame<T>(version, stream as StreamBuffer ?? new StreamBuffer(stream), maximumFrameSize);
    }
    ...
    private static Id3v2Frame? ReadFrame(Id3v2Version version, StreamBuffer sb, long maximumFrameSize)
    {
        ...
        var frame = GetFrame(version, identifier);
    ...
    private static T? ReadFrame<T>(Id3v2Version version, StreamBuffer sb, long maximumFrameSize) where T : Id3v2Frame, new()
    {
        ...
        var frame = new T { Version = version, Identifier = identifier };
        return frame.ReadFrame(sb, maximumFrameSize) ? frame : null;
    }
    ...
    private static string? ReadIdentifier(Id3v2Version version, StreamBuffer sb, long maximumFrameSize)
    {
        ...
        var bytesToSkip = GetBytesUntilNextFrame(version, sb, maximumFrameSize);
    ...
    private bool ReadFrame(StreamBuffer sb, long maximumFrameSize)
    {
        ...
        var startPosition = sb.Position - IdentifierFieldLength;
        ...
        var dataSizeFieldLength = GetDataSizeFieldLength(Version);
        var initialSize = sb.ReadBigEndianInt64(dataSizeFieldLength);
        var dataSize = GetFrameSize(Version, initialSize, maximumFrameSize - GetHeaderSize(Version));
        ...
        var headerSize = sb.Position - startPosition;
        ...
        var binaryStream = new BinaryReader(new MemoryStream(data));
        ...
        if (Version is >= Id3v2Version.Id3v230 and < Id3v2Version.Id3v240)
        {
            ...
            {
                _isCompressed = true;
                var decompressedSize = binaryStream.ReadInt32();
        ...
        else if (Version >= Id3v2Version.Id3v240)
        {
            ...
            {
                var synchedDataLength = binaryStream.ReadInt32();
        ...
        var frameDataSize = (int)Math.Max(binaryStream.BaseStream.Length - binaryStream.BaseStream.Position, 0);
        var frameData = new byte[frameDataSize];
        var dataBytesRead = binaryStream.Read(frameData, 0, frameDataSize);
        var bytesLeftInStream = (int)(maximumFrameSize - headerSize - dataSize);
        if ((dataSize > 0) && (bytesLeftInStream > 0) && (dataBytesRead >= frameDataSize))
        {
            // Hold a temporary buffer for all data available.
            var dataBuffer = new StreamBuffer(frameData) { Position = frameData.Length };
            // If there's still data left in the stream which we haven't read yet, do it here.
            // Try to find the start of the next frame.
            var startPositionNextFrame = sb.Position;
            var bytesUntilNextFrame = GetBytesUntilNextFrame(Version, sb, bytesLeftInStream);
            ...
            if (bytesUntilNextFrame > 0)
            {
                ...
                var extraBytesRead = sb.Read(frameData, 0, frameData.Length);
        ...
        if (UseEncryption)
        {
            if (Cryptor != null)
            {
                var decryptedData = Cryptor.Decrypt(EncryptionType, frameData, frameData.Length);
        ...
        if (UseCompression && !_isEncrypted)
        {
            if (Compressor != null)
            {
                var decompressedData = Compressor.Decompress(frameData, frameData.Length);
    ...
    private static long GetBytesUntilNextFrame(Id3v2Version version, Stream stream, long maximumNextFrameSize)
    {
        ...
        var identifierFieldLength = GetIdentifierFieldLength(version);
        var dataSizeFieldLength = GetDataSizeFieldLength(version);
        var flagSizeFieldLength = (version >= Id3v2Version.Id3v230) ? 2 : 0;
        var extraBytesToRead = identifierFieldLength + dataSizeFieldLength + flagSizeFieldLength;

        // This needs to be long...
        long nextFrameSizeValue = 0, nextFrameFlags = 0;
        var nextFrameHeaderLengthRead = 0;
        ...
        var allowedHeaderFlags = (version < Id3v2Version.Id3v240) ? 0x57568 : 0x28751;
        ...
        while (totalBytesRead++ < Math.Min(stream.Length, maximumNextFrameSize))
        {
            ...
            if (nextFrameHeaderLengthRead < (identifierFieldLength + dataSizeFieldLength))
            {
                ...
                var bits = identifierFieldLength + dataSizeFieldLength - nextFrameHeaderLengthRead - 1;
            ...
            if (nextFrameHeaderLengthRead < extraBytesToRead)
            {
                ...
                var bits = extraBytesToRead - nextFrameHeaderLengthRead - 1;
            ...
            var nextFrameSize = GetFrameSize(version, nextFrameSizeValue, maximumNextFrameSize);

            // See if the frame size is valid.
            var startOffsetNextFrame = stream.Position - startPosition - (nextFrameHeaderLengthRead + 1);
