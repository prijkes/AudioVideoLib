...
public sealed partial class ApeTagReader : IAudioTagReader
{
    ...
    public IAudioTagOffset? ReadFromStream(Stream stream, TagOrigin tagOrigin)
    {
        ...
        while (bytesRead < totalSizeItems)
        {
            ...
            var item = ApeItem.ReadFromStream(tag.Version, sb, totalSizeItems - bytesRead);
            if (item != null)
            {
                var parseEventArgs = new ApeItemParseEventArgs(item);
                ...
                if (!parseEventArgs.Cancel)
                {
                    ...
                    if (item != null)
                    {
                        ...
                        var parsedEventArgs = new ApeItemParsedEventArgs(item);
    ...
    private static ApeHeader? ReadHeader(StreamBuffer stream, TagOrigin tagOrigin)
    {
        ...
        if (tagOrigin == TagOrigin.Start)
        {
            ...
            var endPositionHeader = Math.Min(startPositionHeader + ApeTag.HeaderSize, streamLength);
            var hdr = ReadHeader(stream, startPositionHeader, endPositionHeader);
            if (hdr != null)
            {
                return hdr;
            }
        ...
        else if (tagOrigin == TagOrigin.End)
        {
            ...
            var startPositionHeader = Math.Max(startPosition - ApeTag.FooterSize, 0);
            var endPositionHeader = Math.Min(startPositionHeader + ApeTag.FooterSize, streamLength);
            var hdr = ReadHeader(stream, startPositionHeader, endPositionHeader);
            if (hdr != null)
            {
                return hdr;
