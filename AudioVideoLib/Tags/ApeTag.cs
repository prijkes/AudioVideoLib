...
public sealed partial class ApeTag : IAudioTag
{
    ...
    {
        return tag is not null && (ReferenceEquals(this, tag) || ((tag.Version == Version) && (tag.Flags == Flags) && tag.Items.SequenceEqual(Items)));
    }
    ...
    {
        var keys = ApeItem.GetItemKeys(key);
    ...
    public byte[] ToByteArray()
    {
        ...
        {
            foreach (var field in _items.Select(frame => frame.ToByteArray()))
            {
