...
public sealed partial class Id3v2TagReader : IAudioTagReader
{
    ...
    public IAudioTagOffset? ReadFromStream(Stream stream, TagOrigin tagOrigin)
    {
        ...
        var headerOrFooter = ReadHeader(sb, tagOrigin);
        ...
        // Copy header values.
        var tag = new Id3v2Tag(headerOrFooter.Version, headerOrFooter.Flags);
        var isHeader = string.Equals(Id3v2Tag.HeaderIdentifier, headerOrFooter.Identifier, StringComparison.OrdinalIgnoreCase);
        ...
        var totalSizeItems = Math.Min(headerOrFooter.Size, (int)(sb.Length - sb.Position));
        ...
        {
            var unsynchronizedDataSize = totalSizeItems;
            var unsynchronizedData = new byte[unsynchronizedDataSize];
            unsynchronizedDataSize = sb.Read(unsynchronizedData, 0, unsynchronizedDataSize);
            var data = Id3v2Tag.GetSynchronizedData(unsynchronizedData, 0, unsynchronizedDataSize);
    ...
    private static Id3v2Header? ReadHeader(StreamBuffer stream, TagOrigin tagOrigin)
    {
        ...
        if (tagOrigin == TagOrigin.Start)
        {
            ...
            var endPositionHeader = Math.Min(startPositionHeader + Id3v2Tag.HeaderSize, streamLength);
            var header = ReadHeader(stream, startPositionHeader, endPositionHeader, HeaderIdentifierBytes);
            if (header != null)
            {
                return header;
            }
        ...
        else if (tagOrigin == TagOrigin.End)
        {
            ...
            var startPositionHeader = Math.Max(startPosition - Id3v2Tag.FooterSize, 0);
            var endPositionHeader = Math.Min(startPositionHeader + Id3v2Tag.FooterSize, streamLength);
            var footer = ReadHeader(stream, startPositionHeader, endPositionHeader, FooterIdentifierBytes);
            if (footer != null)
            {
                return footer;
            }
    ...
    private static Id3v2Header? ReadHeader(StreamBuffer stream, long startHeaderPosition, long endHeaderPosition, byte[] identifierBytes)
    {
        ...
        while (startHeaderPosition < endHeaderPosition)
        {
            ...
            while (stream.ReadByte() == identifierBytes[y++])
            {
                ...
                var majorRevision = stream.ReadByte();
                var revisionNumber = stream.ReadByte();
    ...
    private static Id3v2ExtendedHeader? ReadExtendedHeader(StreamBuffer streamBuffer, Id3v2Tag tag, out int crcData)
    {
        ...
        var startPosition = streamBuffer.Position;
        Id3v2ExtendedHeader? extendedHeader = null;
        if (tag.Version is >= Id3v2Version.Id3v230 and < Id3v2Version.Id3v240)
        {
            ...
            var extendedFlags = streamBuffer.ReadBigEndianInt16();
        ...
        else if (tag.Version >= Id3v2Version.Id3v240)
        {
            ...
            var extendedFlagsFieldLength = streamBuffer.ReadByte();

            // The extended flags field, with its size described by 'number of flag bytes', is defined as: %0bcd0000
            var extendedFlags = streamBuffer.ReadInt(extendedFlagsFieldLength);
            extendedHeader = Id3v2ExtendedHeader.InitExtendedHeader(tag.Version, extendedFlags);
            ...
            if (extendedHeader.CrcDataPresent)
            {
                var crcBytes = streamBuffer.ReadBigEndianInt64(5);
    ...
    private static void AddRequiredFrames(Id3v2Version version, ICollection<Id3v2Frame> frames)
    {
        ...
        var musicCdIdentifier = Id3v2Frame.GetIdentifier<Id3v2MusicCdIdentifierFrame>(version);
        var musicCdFrame =
        ...
        {
            var lengthIdentifier = Id3v2TextFrame.GetIdentifier(version, Id3v2TextFrameIdentifier.Length);
            Id3v2Frame? lengthFrame =
                frames.OfType<Id3v2TextFrame>()
                    .FirstOrDefault(f => string.Equals(f.Identifier, lengthIdentifier, StringComparison.OrdinalIgnoreCase));

            var audioSeekPointIndexIdentifier = Id3v2Frame.GetIdentifier<Id3v2AudioSeekPointIndexFrame>(version);
            var audioSeekPointIndexFrame =
                frames.FirstOrDefault(
                    f =>
