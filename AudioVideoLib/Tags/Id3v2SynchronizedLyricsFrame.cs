...
public sealed class Id3v2SynchronizedLyricsFrame : Id3v2Frame
{
    ...
    private string _language = null!, _contentDescriptor = null!;
    ...
    public override byte[] Data
    {
        get
        {
            ...
            // Syncs
            var syncIdentifier = encoding.GetBytes("\0");
        ...
        protected set
        {
            ...
            {
                var syllable = stream.ReadString(encoding);
                ...
                var timeStamp = (stream.Position < stream.Length) ? stream.ReadBigEndianInt32() : 0;
    ...
    public override string Identifier => GetIdentifierFromFactory() ?? "SYLT";
    ...
    {
        return sl is not null && (ReferenceEquals(this, sl) || ((sl.Version == Version) && string.Equals(sl.Language, Language, StringComparison.OrdinalIgnoreCase)
    ...
    {
        var maxAllowedOrderedContentType = Version < Id3v2Version.Id3v230
            ? Id3v2ContentType.Chord
            : Version < Id3v2Version.Id3v240 ? Id3v2ContentType.Trivia : Id3v2ContentType.ImagesUrls;

        return Enum.TryParse(contentType.ToString(), true, out Id3v2ContentType _) && (contentType <= maxAllowedOrderedContentType);
    }
    ...
    private void LyricSyncAdd(object? sender, ListItemAddEventArgs<Id3v2LyricSync> e)
    {
        ...
        for (var i = 0; i < _lyricSyncs.Count; i++)
        {
            var lyricSync = _lyricSyncs[i];
