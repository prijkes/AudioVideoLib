...
public sealed partial class StreamBuffer
{
    ...
    private int Read(byte[] buffer, int offset, int count, bool movePosition)
    {
        ...
        if (buffer == null)
        {
            ...
            var newPosition = _stream.Seek(count, SeekOrigin.Current);
    ...
    private short ReadInt16(bool movePosition, out int bytesRead)
    {
        ...
        return bytesRead == 0 ? (short)-1 : BitConverter.ToInt16(buffer, 0);
    }
    ...
    private int ReadInt32(bool movePosition, out int bytesRead)
    {
        ...
        return bytesRead == 0 ? -1 : BitConverter.ToInt32(value, 0);
    }
    ...
    private long ReadInt64(int length, bool movePosition, out int bytesRead)
    {
        ...
        bytesRead = Read(value, length, movePosition);
        return bytesRead == 0 ? -1 : BitConverter.ToInt64(value, 0);
    }
    ...
    private float ReadFloat(bool movePosition, out int bytesRead)
    {
        var value = new byte[Int32Size];
        bytesRead = Read(value, Int32Size, movePosition);
        return bytesRead == 0 ? -1 : BitConverter.ToSingle(value, 0);
    }
    ...
    private double ReadDouble(bool movePosition, out int bytesRead)
    {
        ...
        return bytesRead == 0 ? -1 : BitConverter.ToDouble(value, 0);
    }
    ...
    private string ReadString(Encoding encoding, string customStringTerminator, bool ignoreByteOrderMarker, bool movePosition, out int bytesRead)
    {
        ...
        var byteOrderMarkerBuffer = new byte[8];
        ...
        var preamble = encoding.GetPreamble();
        var preambleLength = ((preamble.Length > 0) && (bytesRead >= preamble.Length)) ? preamble.Length : 0;
        if (preambleLength > 0)
        {
            for (var i = 0; i < preambleLength; i++)
            {
        ...
        if (preambleLength == 0)
        {
            foreach (var e in EncodingsWithPreambleList)
            {
                var encodingPreamble = e.GetPreamble();
        ...
        var remainder = MinBufferSize % maxByteCount;
        remainder = (remainder == 0) ? maxByteCount : remainder;
        var bufferSize = (MinBufferSize - remainder) * remainder;
